...
function run(options) {
  ...
  function handleValue(value) {
    ...
    const isJSON = editor.getSession().getMode().$id === 'ace/mode/json'
    if (isJSON) {
      try {
        curData.text = value
        var obj = json5.parse(value)
        curData.format = true
        curData.jsonData = obj
        // curData.mockData = () => Mock.mock(obj, {}) // 为防止时时 mock 导致页面卡死的问题，改成函数式需要用到再计算
      } catch (e) {
  ...
  editor.getSession().setMode(options.mode || 'ace/mode/javascript')
